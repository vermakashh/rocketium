...
const LiveCanvas = forwardRef(({ canvasRef, width, height, elements, setElements, setSelectedTextId, selectedColor }, ref) => {
  ...
  const getElementAt = (x, y) => {
    for (let i = elements.length - 1; i >= 0; i--) {
      ...
      if (el.type === "circle") {
        ...
        const offsetY = el.size || 20;
        if (x >= el.x && x <= el.x + el.width && y >= el.y - offsetY && y <= el.y - offsetY + el.height) return el;
  ...
  const getHandleAt = (x, y, el) => {
    ...
    const box = el.type === "circle"
      ? { x: el.x, y: el.y, width: el.radius * 2, height: el.radius * 2 }
      : isText
      ? { x: el.x, y: el.y - (el.size || 20), width: el.width, height: el.height }
    ...
    for (let key in handles) {
      ...
      if (x >= hx - HANDLE_SIZE && x <= hx && y >= hy - HANDLE_SIZE && y <= hy) {
        return key;
      }
    }
    return null;
        ...
        if (resizing) {
          switch (resizeHandle) {
            case "bottomRight":
              ...
              if (el.type === "circle") {
                updated.radius = Math.max(updated.width, updated.height) / 2;
              }
              break;
          }
        } else if (dragging) {
